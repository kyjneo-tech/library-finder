...
export function SituationCategories({ onCategorySearch }: SituationCategoriesProps) {
  return (
    <section className="mx-4 mt-6 mb-8">
      ...
      <p className="text-xs text-gray-500 mb-3">우리 아이에게 딱 맞는 순간을 선택해보세요</p>
      ...
      <div className="grid grid-cols-3 gap-2">
        {SITUATION_CATEGORIES.map((cat) => (
          ...
          >
            ...
            <div className="text-[10px] text-purple-600 mt-0.5">{cat.description}</div>
            {cat.ageRecommend && (
              <div className="text-[9px] text-gray-400 mt-1">{cat.ageRecommend}</div>
            )}
          </button>
        ))}
      </div>
