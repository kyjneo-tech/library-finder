...
export function LocalPopularBooks({ onBookSelect }: LocalPopularBooksProps) {
  ...
  const { getRegionCode, getDisplayName, selectedRegion, selectedSubRegion, selectedDistrict } =
    useRegionStore();
    ...
    return (
      <section className="mx-4 mt-6">
        <h2 className="text-lg font-bold mb-3 flex items-center gap-2">
          🏆 우리 동네 인기 어린이 도서
          <span className="text-sm font-normal text-gray-500">(최근 30일)</span>
        </h2>
  ...
  return (
    <section className="mx-4 mt-6">
      <div className="flex items-center justify-between mb-3">
        <h2 className="text-lg font-bold flex items-center gap-2">🏆 우리 동네 인기 어린이 도서</h2>
      </div>
      ...
      {books.length === 0 ? (
        <div className="p-6 bg-gray-50 rounded-lg border border-gray-200 text-center">
          <p className="text-sm text-gray-600 mb-2">📭 데이터를 불러오지 못했어요</p>
          <button
